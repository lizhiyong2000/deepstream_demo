...
#define PGIE_CONFIG_FILE                                                       \
    "../config/line_crossing/config_infer_primary_yoloV4.txt"
//#define PGIE_CONFIG_FILE "nvdsanalytics_pgie_config.txt"
#define MSCONV_CONFIG_FILE "../config/line_crossing/dstest4_msgconv_config.txt"
#define ANALYTICS_CONFIG_FILE "../config/line_crossing/config_nvdsanalytics.txt"
#define TRACKER_CONFIG_FILE "../config/line_crossing/tracker_config.yml"
...
GstElement *pipeline = NULL, *streammux = NULL, *sink = NULL, *pgie = NULL,
           *nvtracker = NULL, *nvdsanalytics = NULL, *nvvidconv = NULL,
...
static char *ReplaceSubStr(char *str, const char *srcSubStr,
    ...
    if (pFile == NULL)
        ...
        while (fgets(txt_line, 200, pFile) != NULL) {
            ...
            if (strstr(txt_line, "class-id")) {
                ...
                if (lc_class_id == 0) {
                    g_lc_type[lc_stream_id] = g_strdup("Person");
                } else {
                    g_lc_type[lc_stream_id] = g_strdup("Null");
                }
                // g_lc_type[lc_stream_id] =
                // g_strdup(pgie_classes_str[lc_class_id]);
            }
        }
    }
    fclose(pFile);
...
static void generate_event_msg_meta(gpointer data, MyUserMeta *user_param) {
    ...
    //   meta->lccum_cnt_type = g_strdup(user_param->lcc_cnt_type);
}

/* nvdsanalytics_src_pad_buffer_probe  will extract metadata received on tiler
 * sink pad and extract nvanalytics metadata etc. */
...
nvdsanalytics_src_pad_buffer_probe(GstPad *pad, GstPadProbeInfo *info,
    ...
    for (l_frame = batch_meta->frame_meta_list; l_frame != NULL;
        ...
        for (NvDsMetaList *l_user = frame_meta->frame_user_meta_list;
            ...
            user_data[stream_id].lcc_cnt_type =
                g_strdup(g_lc_type[frame_meta->pad_index]);
            ...
            /*
            if (meta->objLCCurrCnt["Entry"] == 0 && meta->objLCCurrCnt["Exit"]
            == 0){ continue;
            }
            else {
              isCrossed = TRUE;
              if (meta->objLCCumCnt["Entry"] <
            user_data[stream_id].lcc_cnt_entry) {
                user_data[stream_id].lcc_cnt_entry +=
            meta->objLCCurrCnt["Entry"];
              }
              else {
...
static gboolean reset_source_pipeline(gint index) {
    // gint index = (gint) i;
    // gint index = GPOINTER_TO_INT (i);
    GstElement *src_elem = g_source_bin_list[index];
    GstState state, pending;
    GstStateChangeReturn ret;

    g_print("Setting source %d to NULL \n", index);

    if (gst_element_set_state(src_elem, GST_STATE_NULL) ==
        GST_STATE_CHANGE_FAILURE) {
        GST_ERROR_OBJECT(src_elem, "Can't set source bin to NULL");
        return FALSE;
    }

    if (!gst_element_sync_state_with_parent(src_elem)) {
        g_print("Couldn't sync state with parent\n");
    }

    /*
    ret = gst_element_get_state (src_elem, &state, &pending, 0);

    g_print("state: %s pending: %s ret: %s\n",
    ...
    */
    ...
    g_print("Setting source %d to PLAYING \n", index);

    ret = gst_element_set_state(src_elem, GST_STATE_PLAYING);
    ...
    if (ret == GST_STATE_CHANGE_ASYNC || ret == GST_STATE_CHANGE_NO_PREROLL) {
        g_timeout_add(20, watch_source_async_state_change, src_elem);
    } else if (ret == GST_STATE_CHANGE_SUCCESS && state == GST_STATE_PLAYING) {
        g_print("GST_STATE_CHANGE_SUCCESS && GST_STATE_PLAYING\n");
    }
    ...
    switch (ret) {
      ...
      case GST_STATE_CHANGE_ASYNC:
        ...
        ret =
            gst_element_get_state (src_elem, NULL, NULL,
            GST_CLOCK_TIME_NONE);
        break;
...
static gboolean bus_call(GstBus *bus, GstMessage *msg, gpointer data) {
    ...
    case GST_MESSAGE_ERROR: {
        ...
        if (g_strrstr(error->message, "The stream is in the wrong format")) {
            g_free(debug);
            g_error_free(error);
            break;
        }
        ...
        GstElement *msg_src_elem = (GstElement *)GST_MESSAGE_SRC(msg);
        gboolean bin_found = FALSE;

        while (msg_src_elem && !bin_found) {
            for (i = 0; i < g_num_sources && !bin_found; i++) {
                // g_print("msg_src_element: %s\n",
                // GST_OBJECT_NAME(msg_src_elem)); g_print("looking %d - %s\n",
                // i, GST_OBJECT_NAME (g_source_bin_list[i]));
                if (g_source_bin_list[i] == msg_src_elem) {
                    g_print("Found the source bin with error: %s\n",
                            GST_OBJECT_NAME(g_source_bin_list[i]));
                    bin_found = TRUE;
                    break;
                }
            }
        ...
        if (i != g_num_sources) {
            if (g_strrstr(error->message, "Could not read from resource")) {
                g_print("reset source\n");
                // stop_release_source(i);
                // add_sources((gpointer) g_source_bin_list, g_video_source[i],
                // i);

                reset_source_pipeline(i);
                // g_free (debug);
                // g_free (error);
                // return TRUE;
            }
            if (g_strrstr(error->message, "Could not open resource")) {
                g_print("reset source\n");
                // stop_release_source(i);
                // add_sources((gpointer) g_source_bin_list, g_video_source[i],
                // i);
            ...
            if (g_strrstr(error->message, "Could not write to resource")) {
                g_print("reset source\n");
                reset_source_pipeline(i);
            }
        }
    ...
    case GST_MESSAGE_ELEMENT: {
        if (gst_nvmessage_is_stream_eos(msg)) {
            ...
            if (gst_nvmessage_parse_stream_eos(msg, &stream_id)) {
                ...
                // g_video_source[stream_id], stream_id);
                reset_source_pipeline(stream_id);
 ...
 */
static gboolean watch_source_async_state_change(gpointer data) {
    GstElement *src_elem = (GstElement *)data;
    GstState state, pending;
    GstStateChangeReturn ret;
    ...
    // state
    g_print("Seting to PLAYING\n");
    gst_element_set_state(src_elem, GST_STATE_PLAYING);
...
static void decodebin_child_added(GstChildProxy *child_proxy, GObject *object,
    ...
    if (g_strrstr(name, "source") == name) {
        // g_print("set TCP done\n");
        g_object_set(G_OBJECT(object), "protocols", 0x00000004, NULL);
        g_object_set(G_OBJECT(object), "latency", 2000, NULL);
...
static void stop_release_source(gint source_id) {
    ...
    g_object_set(G_OBJECT(nvdsanalytics), "config-file", ANALYTICS_CONFIG_FILE,
                 NULL);
    g_print("Setting source %d to null \n", source_id);
    state_return =
...
static gboolean add_sources(gpointer data, gchar *source_uri, gint source_id) {
    ...
    ret = gst_element_get_state(g_source_bin_list[source_id], &state, &pending,
                                0);
    ...
    if (ret == GST_STATE_CHANGE_ASYNC || ret == GST_STATE_CHANGE_NO_PREROLL) {
        g_timeout_add(20, watch_source_async_state_change,
                      g_source_bin_list[source_id]);
    } else if (ret == GST_STATE_CHANGE_SUCCESS && state == GST_STATE_PLAYING) {
        g_print("GST_STATE_CHANGE_SUCCESS && GST_STATE_PLAYING\n");
    }
...
static gboolean delete_sources(gpointer data, gint source_id_delete) {
    ...
    g_video_source[source_id_delete] = g_strdup("-");
...
static gboolean update_source_status(gpointer data) {
    GstState state, pending;
    GstStateChangeReturn ret;
    guint i = 0;
    FILE *pFile = fopen(VIDEO_STATUS_FILE, "w");
    if (pFile == NULL)
        perror("Error opening file");
    else {
        for (i = 0; i < g_num_sources; i++) {
            ret = gst_element_get_state(g_source_bin_list[i], &state, &pending,
                                        0);
            if (g_strrstr(gst_element_state_get_name(state), "PLAYING")) {
                fputs(g_video_source[i], pFile);
                fputs("\n", pFile);
            }
        }
        fclose(pFile);
...
static gboolean read_source_txt(gchar *sourcefile) {
    ...
    */
    ...
    if (pFile == NULL)
        ...
        while (fgets(txt_line, 300, pFile) != NULL) {
            int len = strlen(txt_line);
            // g_print("len: %d", len);
            if (txt_line[len - 1] == '\n')
                txt_line[len - 1] = '\0';
            gchar *tmp = (char *)g_malloc0(300 * sizeof(gchar));
            memcpy(tmp, txt_line, len);
            // g_print("tmp: %s", tmp);
            // tmp: video source url in sources.txt, "-" means delete
...
int main(int argc, char *argv[]) {
    ...
    pipeline = gst_pipeline_new("line-crossing-pipeline");
    ...
    if (!pgie || !nvtracker || !nvdsanalytics || !tiler || !nvvidconv ||
        !nvosd || !msgconv || !msgbroker || !tee || !sink || !queue1 ||
    ...
    /* Configure the nvtracker element for using the particular tracker
     * algorithm.
     */
    g_object_set(G_OBJECT(nvtracker), "ll-lib-file",
                 "/opt/nvidia/deepstream/deepstream-5.0/lib/libnvds_nvdcf.so",
                 "ll-config-file", TRACKER_CONFIG_FILE, "tracker-width", 640,
                 "tracker-height", 480, NULL);

    /* Configure the nvdsanalytics element for using the particular analytics
     * config file*/
    g_object_set(G_OBJECT(nvdsanalytics), "config-file", ANALYTICS_CONFIG_FILE,
                 NULL);
    ...
    sources (%d)\n", pgie_batch_size, num_sources); g_object_set (G_OBJECT
    (pgie), "batch-size", num_sources, NULL);
    }*/
    ...
    gst_bin_add_many(GST_BIN(pipeline), queue1, pgie, queue2, nvtracker, queue3,
                     nvdsanalytics, queue4, tiler, queue5, nvosd, tee, msgconv,
                     msgbroker, nvvidconv, queue6, queue7, queue8, transform,
                     sink, NULL);
    ...
    gst_bin_add_many(GST_BIN(pipeline), queue1, pgie, queue2, nvtracker, queue3,
                     nvdsanalytics, queue4, tiler, queue5, nvosd, tee, msgconv,
                     msgbroker, nvvidconv, queue6, queue7, queue8, sink, NULL);
     ...
     * to the sink pad of the nvdsanalytics element, since by that time, the
     * buffer would have had got all the metadata.
     */
    ...
    // save dot file
    GST_DEBUG_BIN_TO_DOT_FILE((GstBin *)pipeline, GST_DEBUG_GRAPH_SHOW_ALL,
                              "pipeline");
    ...
    g_print("Running...\n");
    // g_timeout_add_seconds(20, (GSourceFunc)read_source_txt,
    // VIDEO_SOURCE_FILE);
    // // g_timeout_add_seconds (50, check_eos_source, (gpointer)
    // // g_source_bin_list);
    // g_timeout_add_seconds(20, (GSourceFunc)update_source_status,
